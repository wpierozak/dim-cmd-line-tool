...
public:
   ...
   * @param timeout Optional timeout value in milliseconds. Default is
   * std::nullopt.
   */
  ...
  /**
   * @brief Returns the number of available values fetched from the service
   * since last clearData() call.
   ...
   * @param output Specifies the terminal output to be printed. Default is
   * TerminalOutput::Latest.
   */
...
protected:
  bool
  checkTimeout(const std::chrono::high_resolution_clock::time_point &startTime,
    ...
    return std::chrono::duration_cast<std::chrono::milliseconds>(currentTime -
                                                                 startTime)
               .count() > m_timeout.value();
  }
  uint32_t timeout() { return m_timeout.value_or(0); }
