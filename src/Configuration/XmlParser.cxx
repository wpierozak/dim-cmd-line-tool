...
void XmlParser::populateDimManager(std::string configPath)
{
    ...
    if (!root)
    {
        throw std::runtime_error("Missing <Configuration> root node in config file.");
    }
    ...
    for (auto* serviceNode = root->first_node("Service"); serviceNode; serviceNode = serviceNode->next_sibling("Service"))
    {
        ...
        } else if(serviceInfo.type == "service_info" || serviceInfo.type == "rpc_info"){
...
{
    auto senderRes = dim_handlers::DimManager::Instance().createCommandSender(serviceInfo.name, serviceInfo.alias);
    if (senderRes.isOk())
    {
        ...
        if (serviceInfo.responseOn)
        {
            ...
            {
                dim_handlers::DimManager::Instance().addResponseServiceToCommandSender(serviceInfo.name, respService);
            }
        }
    }
}
...
{
    auto type = serviceInfo.type == "service_info" ? dim_handlers::Subscriber::Type::ServiceInfo : dim_handlers::Subscriber::Type::RpcInfo;

    if(serviceInfo.output.has_value()){ 
        dim_handlers::DimManager::Instance().createSubscriber(
            serviceInfo.name,
            serviceInfo.alias,
            serviceInfo.output->filename,
            type,
