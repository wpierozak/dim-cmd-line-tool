...
config::ServiceInfo::Commands::Commands(rapidxml::xml_node<> *node) {
  for (rapidxml::xml_node<> *cmd = node->first_node("command"); cmd;
       cmd = cmd->next_sibling("command")) {
...
config::ServiceInfo::ServiceInfo(rapidxml::xml_node<> *node) {
  ...
  for (rapidxml::xml_node<> *child = node->first_node(); child;
    ...
    if (std::strcmp(child->name(), "output") == 0) {
      ...
      Commands cmds(child);
      commands.insert(commands.end(), cmds.commands.begin(),
                      cmds.commands.end());
    } else if (std::strcmp(child->name(), "response_on") == 0) {
      ResponseOn resp;
      for (rapidxml::xml_node<> *srv = child->first_node("service"); srv;
           srv = srv->next_sibling("service")) {
